...
- [~] **T6.3 — Themes & status indicators**
  ...
  - **Delivered:**
    ...
    - **Model switcher in the AI chat panel:** the chat panel now has a **provider tag + model dropdown** row (below the "⚙ settings" link). It lists **every model across all provider presets plus the currently saved model** (deduped, derived from `AI_PROVIDERS` + saved `aiModel`). Picking a model that belongs to a preset **auto-switches the provider + base URL** (e.g. `deepseek-chat` → DeepSeek preset, `claude-sonnet-…` → Anthropic), then persists via `set_ai_config` — "like a traditional settings screen", the provider is just shown as a tag so you never have to jump into Settings for a quick model change. Dropdown styled dark (`color-scheme: dark`, WebKitGTK-safe). Verified in browser: saved `jandelcombo` shown, switching to `deepseek-chat`/`claude-sonnet-4-20250514` updates the provider tag live; svelte-check 0/0, build OK. (commit `2f62a12`)
