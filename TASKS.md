...
- [~] **T6.3 — Themes & status indicators**
  ...
  - **Delivered:**
    ...
    - **Multi-provider AI (T6.3 refactor):** the Settings modal now has an **AI provider** selector — **Custom (OpenAI-compatible)**, **DeepSeek** (preset `https://api.deepseek.com/v1`), and **Claude (Anthropic)** (preset `https://api.anthropic.com/v1`). Presets prefill endpoint + default model (model input has a per-provider datalist). Backend `ai.rs` dispatches by provider: OpenAI-compatible `/chat/completions` for openai+deepseek, Anthropic **Messages** `/v1/messages` for claude (full tool-calling: converts history to Anthropic messages incl. grouping `tool_result`s, normalizes `tool_use` blocks back to the shared OpenAI-style `ChatResponse`). **API key is encrypted at rest** (ChaCha20-Poly1305 keyed from the machine id; `ai.json` stores only `api_key_enc`, plaintext never on disk; legacy plaintext configs still load). Verified: encryption roundtrip + key-at-rest test, live OpenAI call against 192.168.5.52:20128 still works, cargo 7/7, browser provider dropdown prefills presets.
    - **Model switcher in the AI chat panel:** the chat panel now has a **provider tag + model dropdown** row (below the "⚙ settings" link). It lists **every model across all provider presets plus the currently saved model** (deduped, derived from `AI_PROVIDERS` + saved `aiModel`). Picking a model that belongs to a preset **auto-switches the provider + base URL** (e.g. `deepseek-chat` → DeepSeek preset, `claude-sonnet-…` → Anthropic), then persists via `set_ai_config` — "like a traditional settings screen", the provider is just shown as a tag so you never have to jump into Settings for a quick model change. Dropdown styled dark (`color-scheme: dark`, WebKitGTK-safe). Verified in browser: saved `jandelcombo` shown, switching to `deepseek-chat`/`claude-sonnet-4-20250514` updates the provider tag live; svelte-check 0/0, build OK. (commit `2f62a12`). **Fixed (234560e):** the custom OpenAI-compatible endpoint is now remembered (`customBaseUrl`) so picking a custom-provider model after trying DeepSeek/Claude restores it instead of leaving a stale preset URL (which sent the custom key to `api.deepseek.com` → 401).
    - **Save toast:** clicking **Save** in Settings now shows a toast — `AI settings saved — <provider> · <model>` (auto-hides after 3.5s), plus a red error toast on failure. (commit `d990d00`)
    - **Install-agent hint on recalled ssh commands:** the install hint + button now also appear when an `ssh …` command was **recalled from shell history (up-arrow)** — the pty output is scanned for `ssh <target>` once the shell is back at a prompt (connection established), so host detection no longer depends solely on raw typed keys. (commit `d990d00`)
...
- [x] **T6.7 — Agentic AI safety: abort, guardrails, activity**
  ...
  - **Delivered:**
    ...
    - 🐛 **Abort now REALLY stops the remote (live-verified fix):** killing the local ssh alone does NOT stop the remote command — sshd keeps the session's process running after the connection drops (verified empirically; also means a crashed client leaves the command running). Heartbeat-on-stdout was tried and reverted (sshd holds the agent's stdout pipe open, so writes never fail). Real fix: the agent tracks its running command's process group (`action.ActiveCommandPID`), and on **SIGTERM/SIGHUP kills that group** (`pkill -TERM`); the client's Abort now ALSO issues a remote `pkill -TERM -u <user> -f puppetterm-agent` over a fresh SSH connection. Verified live on server1: abort kills the agent AND the running command (`sh -c "sleep 90"` group). Known limitation: if the client itself vanishes (crash/network, not Abort), the remote command still runs to completion.
...
- [x] **T6.8 — In-app agent installer (Warp-style)**
  ...
  - 🐛 **Password-only remotes (committed `fbf0d38`, Include-position fix `facb7be`, ControlMaster `auto` fix `8a85c70`):** hosts with no local SSH key (password auth) previously failed every automated call (`Permission denied (publickey,password)`) because the app's ssh calls were non-interactive. Now the app **idempotently enables ssh ControlMaster sharing** at startup (and before install): writes `~/.ssh/puppetterm-control` (`Host * ControlMaster auto`, `ControlPath ~/.ssh/puppetterm-mux/%r@%h:%p`, `ControlPersist 600`) and `Include`s it from `~/.ssh/config` (creating both as needed, never clobbering user entries; also creates the mux dir since ssh won't). The user's interactive `ssh user@host` (password typed once) becomes the **master**; every automation ssh call (`agent.rs` run_action, `install.rs` check_agent/ssh_io, `lib.rs` stop_agent_action, `ssh.rs` check_host) now passes **`-o ControlMaster=auto`** so it **attaches** to that socket instead of re-authing — and still falls back to a direct key-based connection when no master exists. **Gotchas found live:** (1) the `Include` MUST be at the **top** of `~/.ssh/config` — OpenSSH's `Host *` matching is broken for non-alias targets (`ssh -G isr@192.168.150.22` → `controlmaster false`) when the Include sits after a `Host` block (fixed `facb7be`). (2) **`ControlMaster yes` is WRONG** — it makes every ssh try to be the master; with an existing socket it prints "ControlSocket … already exists, disabling multiplexing" and then either connects directly (re-prompting the password) or, with a broken/stale master, fails with "PTY allocation request failed on channel 0" — breaking the user's next interactive ssh after an install (the install's automation master can linger as a stale socket). **`ControlMaster auto`** instead: creates the master on the first connection, ATTACHES on later ones (no re-prompt), and falls back to a normal connection when the socket is stale (fixed `8a85c70`). **Hint timing:** the agent check/hint was moved off the onData (typed) path and now only runs from the **prompt-gated buffer scan** (once the connection is established). Verified: `ssh -G` matrix; localhost 1st=create, 2nd=attach ("Shared connection"), stale-socket=graceful fallback; unit test `ssh_control_master_config_is_idempotent` (8/8); browser mock hint + button for `staging`. Wording updated to "reuses your SSH connection".
  - 🐛 **Idempotent + self-contained (live fix):** the installer previously failed with "agent binary not found at ~/.puppetterm/agents" (it only looked in env/home). Now the agent binaries are **bundled as Tauri resources** (`bundle.resources` → `../../agent/bin/*`), resolved as param → `PUPPETTERM_AGENT_DIR` env → **source-tree `<repo>/agent/bin` first (dev, `make cross` output)** → bundled resource dir (packaged) → home. In `tauri dev` the resource dir is `target/debug` which never has the binary, so the source-tree `agent/bin` must win when present (fixed `955390e` — previously installs failed "agent binary not found at .../target/debug/..."). The install is **idempotent**: an existing root agent at `/usr/local/bin` is detected and skipped ("already installed (root install) — nothing to do"); an existing user-space agent is refreshed in place. `InstallResult.already` flags it and the UI shows "already present —". Verified live on server1: existing root agent detected, `already=true`, no duplicate key/install. If install reports "binary not found" after a fresh clone/reboot, run `cd agent && make cross` (needs go on PATH).
