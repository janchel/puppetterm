...
- [x] **T6.8 — In-app agent installer (Warp-style)**
  ...
  - 🐛 **Password-only remotes (committed `fbf0d38`, Include-position fix `facb7be`, ControlMaster `auto` fix `8a85c70`, broken-master cleanup `d6b0858`):** hosts with no local SSH key (password auth) previously failed every automated call (`Permission denied (publickey,password)`) because the app's ssh calls were non-interactive. Now the app **idempotently enables ssh ControlMaster sharing** at startup (and before install): writes `~/.ssh/puppetterm-control` (`Host * ControlMaster auto`, `ControlPath ~/.ssh/puppetterm-mux/%r@%h:%p`, `ControlPersist 600`) and `Include`s it from `~/.ssh/config` (creating both as needed, never clobbering user entries; also creates the mux dir since ssh won't). The user's interactive `ssh user@host` (password typed once) becomes the **master**; every automation ssh call (`agent.rs` run_action, `install.rs` check_agent/ssh_io, `lib.rs` stop_agent_action, `ssh.rs` check_host) now passes **`-o ControlMaster=auto`** so it **attaches** to that socket instead of re-authing — and still falls back to a direct key-based connection when no master exists. **Gotchas found live:** (1) the `Include` MUST be at the **top** of `~/.ssh/config` — OpenSSH's `Host *` matching is broken for non-alias targets when the Include sits after a `Host` block (fixed `facb7be`). (2) **`ControlMaster yes` is WRONG** (fixed `8a85c70` → `auto`): `auto` creates on first connect, attaches on later, falls back on a stale socket; `yes` makes every ssh try to be a master and breaks on an existing socket. (3) **A ControlPersist master can go half-dead** — answers `-O check` but hangs any new session → the next interactive ssh fails "PTY allocation request failed". `ensure_ssh_control_master` now spawns a **background cleanup** (`cleanup_stale_masters`) that probes each socket (throwaway `ssh -S sock -o ControlMaster=no … true`, 4s timeout) and removes dead/broken ones (fixed `d6b0858`). **Hint timing:** the agent check/hint was moved off the onData (typed) path and now only runs from the **prompt-gated buffer scan** (once the connection is established). Verified: `ssh -G` matrix; localhost 1st=create / 2nd=attach / stale=fallback; cleanup probe keeps healthy masters; unit test `ssh_control_master_config_is_idempotent` (8/8); browser mock hint + button for `staging`. Wording updated to "reuses your SSH connection".
