...
- [~] **T6.3 — Themes & status indicators**
  ...
  - **Delivered:**
    ...
    - Terminal copy/paste: `Ctrl+Shift+C` / `Ctrl+Insert` / right-click auto-copy the selection, `Ctrl+Shift+V` / `Shift+Insert` / right-click (no selection) paste; `onSelectionChange` auto-copies. Uses the **native Tauri clipboard plugin** (`tauri-plugin-clipboard-manager`, arboard backend) with web-API fallback for browser mode — WebKitGTK's `navigator.clipboard` is unreliable, which broke right-click copy in the real window. Right-click captures the selection on `mousedown` (button 2) before the `contextmenu` handler copies/pastes. Verified: browser right-click paste echoes into the terminal; selection → `onSelectionChange` → clipboard write OK.
