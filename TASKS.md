...
- [x] **T6.8 — In-app agent installer (Warp-style)**
  ...
  - 🐛 **Password-only remotes — REVERTED (committed `4e7af14`):** the ssh ControlMaster approach (`Host * ControlMaster auto` written to `~/.ssh/config` so the user's interactive ssh backs password-auth install/actions) kept breaking interactive ssh — stale/half-dead masters caused "PTY allocation request failed" even after the Include-top fix (`facb7be`) and the `auto`-not-`yes` fix (`8a85c70`). **Reverted the whole thing**: no more editing `~/.ssh/config` / `puppetterm-control`; ssh is a plain terminal again for every connection; `ensure_ssh_control_master` + its test removed; `-o ControlMaster=auto` stripped from all automation ssh (plain key-based again); `cleanup_stale_masters` kept to sweep leftover sockets. **Replacement:** the AI now gets a **`terminal` tool** that types a command into the LIVE terminal and waits for the output — works on ANY host (key OR password) because it rides the user's actual pty session (no separate ssh, no ControlMaster). Read-only commands (`ls/cat/pwd/…`) auto-run; everything else is approved; `DANGEROUS_PATTERNS` flagged. System prompt updated to prefer `terminal`. The Go agent ssh tools (run_command/service/log/config/snapshot) remain for key-based hosts. Verified: local shell round-trip in browser mock; svelte-check 0/0, build OK, cargo 7/7. (User's live `~/.ssh/config` was reverted: Include removed, `puppetterm-control` + mux dir deleted.)
  - 🐛 **ssh-agent gotcha (2026-08-10):** "PTY allocation request failed on channel 0" with NO password prompt was actually the **`puppetterm-agent` private key auto-loaded into the ssh-agent by gnome-keyring** (it scans `~/.ssh/`), offered to every host; on password hosts where that key is a command-locked `no-pty` entry, ssh logged in as the forced agent command instead of a shell. Fix: keep the agent PRIVATE key out of `~/.ssh/` (moved to `~/.puppetterm/keys/`; only the `.pub` stays) + `systemctl --user restart gnome-keyring-daemon`. `ssh-add -D` does NOT stick. Verified: only `id_rsa` offered, password host → normal password prompt, server1 key auth still OK.
