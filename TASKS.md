...
- [x] **T6.8 — In-app agent installer (Warp-style)**
  ...
  - 🐛 **Password-only remotes (committed `fbf0d38`):** hosts with no local SSH key (password auth) previously failed every automated call (`Permission denied (publickey,password)`) because the app's ssh calls were non-interactive. Now the app **idempotently enables ssh ControlMaster sharing** at startup (and before install): writes `~/.ssh/puppetterm-control` (`Host * ControlMaster yes`, `ControlPath ~/.ssh/puppetterm-mux/%r@%h:%p`, `ControlPersist 600`) and `Include`s it from `~/.ssh/config` (creating both as needed, never clobbering user entries; also creates the mux dir since ssh won't). The user's interactive `ssh user@host` (password typed once) becomes the **master**; every automation ssh call (`agent.rs` run_action, `install.rs` check_agent/ssh_io, `lib.rs` stop_agent_action, `ssh.rs` check_host) now passes **`-o ControlMaster=auto`** so it **attaches** to that socket instead of re-authing — and still falls back to a direct key-based connection when no master exists. **Hint timing:** the agent check/hint was moved off the onData (typed) path and now only runs from the **prompt-gated buffer scan** (i.e. once the connection is established), so password hosts no longer get a premature "agent not detected" before the user connects, and the check runs when the master socket actually exists. Verified: unit test `ssh_control_master_config_is_idempotent` (8/8 tests); end-to-end on localhost — 1st ssh creates the socket, a `BatchMode` ssh with `-o ControlMaster=auto` attaches (`ATTACHED_VIA_MASTER`), `-O check` → "Master running"; browser mock still shows the hint + button for `staging`. Wording updated to "reuses your SSH connection" instead of "uses your SSH key".
