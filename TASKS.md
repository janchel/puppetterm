...
- [x] **T6.8 — In-app agent installer (Warp-style)**
  ...
  - 🐛 **Idempotent + self-contained (live fix):** the installer previously failed with "agent binary not found at ~/.puppetterm/agents" (it only looked in env/home). Now the agent binaries are **bundled as Tauri resources** (`bundle.resources` → `../../agent/bin/*`), resolved as param → `PUPPETTERM_AGENT_DIR` env → bundled resource dir → source-tree `<repo>/agent/bin` dev fallback → home. The install is **idempotent**: an existing root agent at `/usr/local/bin` is detected and skipped ("already installed (root install) — nothing to do"); an existing user-space agent is refreshed in place. `InstallResult.already` flags it and the UI shows "already present —". Verified live on server1: existing root agent detected, `already=true`, no duplicate key/install.
